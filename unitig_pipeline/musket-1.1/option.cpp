...
void PrintUsage(ProgramOptions& opt) {
	cerr << endl << "MUSKET (version " << VERSION
			<< ") is a parallel multi-stage k-mer based error corrector"
			<< endl;
	...
	cerr << "Basic Options:" << endl
			...
			<< endl << "\t-o <str> (the single output file name)" << endl
			<< "\t-omulti <str> (prefix of output file names, one input corresponding one output)"
			<< endl << "\t-p <int> (number of threads [>=2], default="
			<< opt.nthreads << ")" << endl
			<< "\t-zlib <int> (zlib-compressed output, default=" << opt.compress
			<< ")" << endl
...
void ParseOptions(int argc, char **argv, ProgramOptions &opt) {
	...
	while (index < argc) {
		...
		if (!strcmp(argv[index], "-k")) {
			...
			index++;
		} else if (!strcmp(argv[index], "-zlib")) {
			index++;
			if (index < argc) {
				sscanf(argv[index], "%d", &value);
				if (value < 2) {
				...
				opt.compress = value == 0 ? false : true;
			} else {
				cerr << "not specify value for option " << argv[index - 1]
						<< endl;
				PrintUsage(opt);
			...
			index++;
		} else if (!strcmp(argv[index], "-minmulti")) {
			index++;
			if (index < argc) {
				sscanf(argv[index], "%d", &value);
				/*set the threshold value*/
				threshold = value < 0 ? -1 : value;
			} else {
				cerr << "not specify value for option " << argv[index - 1]
						<< endl;
				PrintUsage(opt);
			...
			index++;
		} else if (!strcmp(argv[index], "-inorder")
				|| !strcmp(argv[index], "-paired")) {
			opt.keepOrder = true;
			++index;
	...
	if (opt.correctedFileName.length() == 0
			&& correctedFileNamePrefix.length() == 0) {
		cerr
				<< "Must specif the output using either option \"-o\" or \"-omulti\""
				<< endl;
		exit(0);
	}
	if (correctedFileNamePrefix.length() > 0
			&& opt.correctedFileName.length() > 0) {
	...
	/*check the threshold*/
	if (opt.enableMK) {
		opt.watershed.resize(opt.klist.size(), -1);
	} else {
		opt.watershed.resize(opt.klist.size(), threshold);
	}
